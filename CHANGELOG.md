...
### 2026-09-23 — echo-HC v0.04: Solarized Dark, which-key, browser polish, README refresh

- **Solarized Dark theme** across ModernZ, OSD, console / select menus, stats graphs and the pause indicator, one translucency level (alpha `0x50`) with a thin base01 edge on browse, whichkey and console menus; OSD border 1.5. ModernZ local patch `seekbar_shimmer`: animated blue → cyan → green gradient on the seekbar progress.
- **Right-click menu themed**: `load-context-menu=yes` swaps the Win32 popup for mpv's OSD-drawn `context_menu.lua` over the same `menu.conf`; `script-opts/context_menu.conf` gives it the console's Solarized look (base03 pane at alpha 80, base01 edge, base03-on-green focus).
- **Stats on the pane**: `scripts/stats.lua` is mpv's stats script copied from the bundled build's commit (`7b8915bc1`) and loaded instead of the built-in (`load-stats-overlay=no`); a local patch measures the text and draws it on the browse/whichkey pane. `stats.conf`: `persistent_overlay=yes`, `border_size=0`. Re-sync steps in `docs/testing.md`.
- **`whichkey.lua`** (new): <kbd>g</kbd> (select.lua lists) and <kbd>b</kbd> (browse) prefix panels listing the bindings under them; `sub-reload` moved to `g-g`, deband to `d` only.
- **`browse.lua`**: reopen last results (`Ctrl+b`, `b-r`, Open > Reopen results), `Shift+Enter` queues, per-source prompt memory, Twitch live sorted by viewers with uptime and compact viewer counts, offline-only search says so. Torrent rows read `[Group] Show  S2 E07  1080p` with episodes resolved to TVDB `S##E##` via Fribb anime-lists + anime-relations (`browse_anime.lua`, lists refreshed weekly); one AniList lookup per show for cover and MAL id; `Part N` / `Cour N` parsed; thin torrents dimmed.
- **Fixes**: thumbfast spawns for YouTube (EDL trimmed to one small video stream) and no longer scrambles thumbnails after `vsr_autocrop` crops; `[HDR]` stops forcing `d3d11-output-csp`/`-format` (no VO rebuild at HDR start); `hdr-mode` stops re-applying the render target every frame; cropdetect stays in the chain so retries no longer drop frames; `autodeint` filters go before `@vsr` and undo per file; webtorrent overlay stays up until the first frame; `ytdl-format` height cap applies to the muxed fallback; `clip_export` uses `-t`; `open_file` reads dialog output as UTF-8; downmix keeps 5.1(side) surrounds.
- **README** rewritten with GitHub alerts, collapsible sections, a key-binding table and a Built on section crediting the upstream projects. `doc/header.svg` recoloured Solarized Dark with the shimmer seekbar. All screenshots retaken on this build in SDR (a GDI grab of an HDR window blows out OSD colours), plus new which-key and stats shots; Configuration Manager screenshot updated.

### 2026-09-04 — echo-HC v0.03: Torrents source, in-memory streaming through webtorrent-mpv-hook

- **`browse.lua` Torrents source** (Open > Torrents: Search..., New releases, Followed shows; bindings `browse/torrent-search`, `torrent-new`, `torrent-followed`, no default keys). The index is whatever the user pastes into `script-opts/browse.conf`: `torrent_search_url` (RSS search URL with a `{query}` placeholder), `torrent_new_url`, `torrent_shows` (comma-separated search strings, optional `|Group` suffix keeps one release group). The repo names no index; with both URLs empty the submenu is hidden (`menu.conf` reads `user-data/browse/torrents`) and the bindings say which keys to set. Feeds are fetched with Windows `curl`, one request per view, one per followed show merged.
- **`browse_torrents.lua`** (new, pure Lua, no mpv dependency): reads item title / link / enclosure / pubDate and seeders, size, infoHash, trusted by local element name regardless of namespace prefix; plays the `.torrent` link when the feed has one (it carries the tracker list; a magnet built from the bare info hash found no peers via DHT alone), else a magnet in link or enclosure, else a magnet from the info hash. Title parser for fansub naming (`[Group] Show - NN (1080p)`, `SNNENN`, bare ` NN (`, trailing ` - NN`, season batches, 4K remuxes). Ordering: grouped by show, groups newest first, within a show episode desc, resolution desc, seeders desc; releases under a 3-seeder floor sink to the end of their show; unparsed titles form the last group. Missing fields degrade (no seeders: no seeder sort or floor; no trusted: no marker). Unit test `docs/tests/browse_torrents_test.lua` runs under `mpv --no-config --idle=once --script=`.
- **Covers**: one AniList GraphQL search per show name for the grid, first hit's cover, cached in `%TEMP%\mpv-browse-thumbs\cover_<hash>.txt` beside the thumbnails; an empty file records "no cover". Wrong covers are cosmetic.
- **`webtorrent.js`** (webtorrent-mpv-hook 1.4.7, vendored under `webtorrent/` with bun) is now configured: `script-opts/webtorrent.conf` restored with `path=memory`, `port=37346`. A bun patch (`webtorrent/patches/`) replaces the hook's "only one instance is allowed" throw with abort-and-restart, so a second release in the same mpv session kills the first node process and plays; verified with two public torrents (one `node.exe` at a time, nothing written to the working directory). Node.js must be on `PATH`.
- **`Ctrl+Shift+t`** toggles the hook's transfer overlay (`webtorrent/toggle-info`). The hook's default `p` is taken by `sub-fix-timing`; the issue's `Ctrl+t` is already Twitch search.
- Docs: ADR 0011, `CONTEXT.md` glossary, README installation step 4 and troubleshooting entries, `docs/testing.md` torrent recipes.
...
- README rewritten for the echo-HC edition: browser, cookie setup, shader and HDR behaviour, new Troubleshooting entries. `doc/header.svg` retitled with a YouTube + Twitch tag. Screenshots retaken on the current build: player OSC, right-click menu, and two new browser shots (list and grid) replace the Open and Tools submenu shots.
