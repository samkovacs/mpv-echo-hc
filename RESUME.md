...
11. **browse.lua** (2026-09-02): YouTube search + account feeds (subs/history/watch later/home) via yt-dlp `ytsearchN:` and `:ytsubs` etc, shown with `mp.input.select`. Menu: Open > YouTube; key Ctrl+y. Search verified end to end. Feeds verified working with the private-window YouTube cookies in yt-dlp-cookies.txt. Twitch part of browse.lua not started.
