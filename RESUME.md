...
10. **Twitch 1440p** (2026-09-02): Twitch withholds the 1440p60 Source (HEVC) rendition from anonymous requests. mpv.conf `ytdl-raw-options-append=cookies=<repo>/yt-dlp-cookies.txt` (gitignored; twitch.tv + youtube.com cookies exported from Zen, YouTube ones from a closed private window). cookies-from-browser was dropped: yt-dlp writes the whole jar back to the file. Verified 2560x1440 HEVC in mpv. Breaks silently (back to 1080p) after a Twitch logout in Zen; re-export twitch.tv site cookies into the file.
11. **browse.lua** (2026-09-02): YouTube search + account feeds (subs/history/watch later/home) via yt-dlp `ytsearchN:` and `:ytsubs` etc, shown with `mp.input.select`. Menu: Open > YouTube; key Ctrl+y. Search verified end to end. Feeds verified working with the private-window YouTube cookies in yt-dlp-cookies.txt. Twitch: search via GraphQL `searchFor` works (Ctrl+t). Account follow list is unreachable: Twitch answers every third-party GQL request for `follows`/`followedLiveUsers`/FOLLOWED_SECTION with "service error" (tried raw query, browser UA, integrity token, other client ids; Helix 404s for the web client id). Fallback: `script-opts/browse.conf twitch_channels=` list checked with `users(logins:)`, menu Open > Twitch > Live channels. Both verified end to end. Results render on an own ASS overlay: list or 4x3 thumbnail grid (`view=` in browse.conf, Tab toggles; thumbnails = curl download + ffmpeg to raw BGRA cached in %TEMP%mpv-browse-thumbs, drawn with overlay-add; the static ffmpeg cannot do https itself, TLS verify fails). Solarized Dark colors, fuzzy type-to-filter with orange match highlight, Esc clears filter then closes; mouse hover focuses, left click loads); test harness = drive keys via `keypress` from a --script and `screenshot-to-file <png> window`.
