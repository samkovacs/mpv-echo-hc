-- mpv's built-in stats.lua, copied from mpv commit 7b8915bc1 (the bundled
-- mpv.exe build) and loaded instead of it (load-stats-overlay=no). Local
-- patch (echo-HC): the overlay is drawn on the translucent Solarized pane
-- that browse.lua and whichkey.lua use; search "local patch". To re-sync with
-- a newer mpv, take player/lua/stats.lua at the new build's commit and
-- reapply that block. See docs/testing.md.
-- Display some stats.
--
...
-- local patch (echo-HC): measure the text, then draw it on a base03 pane
-- (alpha 0x50, 1px base01 edge), the same backdrop as browse.lua/whichkey.lua.
-- Only used with persistent_overlay=yes (script-opts/stats.conf).
-- Canvas is 288 high like mp.osd_message, which font_size is scaled for;
-- the edge and padding are converted from window pixels.
local pane = mp.create_osd_overlay("ass-events")
pane.res_x, pane.res_y = 0, 288
local function set_pane_ass(data)
    if data == "" then
        pane:remove()
        return
    end
    pane.data, pane.hidden, pane.compute_bounds = data, true, true
    local b = pane:update()
    local px = 288 / math.max(mp.get_property_native("osd-height", 720), 1)
    local pad = 8 * px
    local w, h = b.x1 - b.x0 + 2 * pad, b.y1 - b.y0 + 2 * pad
    pane.data = string.format("{\\an7\\pos(%.2f,%.2f)\\1c&H362B00&\\1a&H50&\\bord%.3f\\3c&H756E58&\\shad0\\p1}m 0 0 l %.2f 0 l %.2f %.2f l 0 %.2f{\\p0}",
                              b.x0 - pad, b.y0 - pad, px, w, w, h, h) .. "\n" .. data
    pane.hidden, pane.compute_bounds = false, false
    pane:update()
end
...
local function clear_screen()
    if o.persistent_overlay then set_pane_ass("") else mp.osd_message("", 0) end
end
